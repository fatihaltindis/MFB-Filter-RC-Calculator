def MFB_LowPass(cutoff, gain = 1, Q = 0.707, margin = 2, filter_type = None, cap = None, res = None, visible='Off'):
    ...
    wc = 2*np.pi*fc_ideal
    C1_value, C2_value = [], []
    R1_value, R2_value, R3_value = [], [], []
    Q_value, Gain_value, fc_value = [], [], []
    for c_temp in cap_library:
        # Start assigning C2 capacitor value
        C2_candid = c_temp

        # Ideal R1 and R2 values
        R1_ideal = (1/Q_ideal)/(2*gain_ideal*wc*C2_candid)
        R2_ideal = (1/Q_ideal)/(2*(gain_ideal+1)*wc*C2_candid)
        R3_ideal = R1_ideal*gain_ideal

        simple_R1 = si_prefix.split(R1_ideal)
        simple_R2 = si_prefix.split(R2_ideal)
        simple_R3 = si_prefix.split(R3_ideal)
        if (simple_R1[1] == 6 and simple_R1[0] >= 10) or simple_R1[1] > 6:
            continue
        elif (simple_R2[1] == 6 and simple_R2[0] >= 10) or simple_R2[1] > 6:
            continue
        elif (simple_R3[1] == 6 and simple_R3[0] >= 10) or simple_R3[1] > 6:
            continue
        else:        
            ...
            R3_candid_base = np.around(np.array(simple_R3[0]),decimals=1)
            R3_candid_power = (10**simple_R3[1])        
            R3_candid = find_nearest(res_library,R3_candid_base*R3_candid_power)

            gain_actual = R3_candid/R1_candid

            # Ideal C1 value
            C1_ideal = (4*Q_ideal**2)*(gain_actual+1)*C2_candid
            ...
            w0 = 1/(R1_candid*R2_candid*C1_candid*C2_candid)

            Q_candid = np.sqrt(R2_candid*R3_candid*C1_candid*C2_candid)/\
            ((R3_candid+R2_candid+R2_candid*gain_actual)*C2_candid)
            fc_candid = np.sqrt(1/(R2_candid*R3_candid*C1_candid*C2_candid))/(2*np.pi)
        ...
        if np.abs(Q_candid-Q_ideal) <= Q_margin:
            ...
            R3_value.append(si_prefix.si_format(R3_candid))
    ...
    selected_values = {'C1':C1_value, 'C2':C2_value,'R1':R1_value,'R2':R2_value,'R3':R3_value,\
